...
export const isSelect = (q: Question) =>
	['select', 'select_one'].includes(qType(q))
export const isText = (q: Question) => qType(q) === 'text'
export const isDate = (q: Question) => qType(q) === 'date'
export const isPhone = (q: Question) =>
	qType(q) === 'phone' || q.code === 'q_phone'
export const isEmailQ = (q: Question) =>
	qType(q) === 'email' || q.code === 'q_email'
...
export const getOptions = (q?: Question) => {
	...
	return raw.map((o: any) => ({
		...
		label:
			o.label ?? o.title ?? o.name ?? String(o.value ?? o.code ?? o.id ?? ''),
	}))
}
